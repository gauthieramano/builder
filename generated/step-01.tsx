...
type Slot = {
  ...
  available: boolean;
  conflictIds: string[]; // bookings overlapping this slot
};
...
function addMinutes(t: string, delta: number) {
  return minutesToHHmm(parseTimeToMinutes(t) + delta);
}
...
function getMonthMatrix(view: Date) {
  ...
  // Start on Monday (ISO). JS getDay(): 0=Sun ... 6=Sat
...
export default function BookingCalendarPage() {
  ...
  const [hours, setHours] = useState<WorkingHours>({ start: "09:00", end: "18:00" });
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-1 h-1.5 w-1.5 rounded-full"
                        style={{ backgroundColor: hasBookings ? "#10b981" : "transparent" }}
                        aria-hidden
                      />
                    </button>
                  );
                ...
                {slots.map((s, idx) => (
                  <button
                    ...
                    className={[
                      ...
                      selectedSlot && selectedSlot.start === s.start && selectedSlot.end === s.end ?
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {s.available ? (
                        <Check className="size-4 opacity-0 group-hover:opacity-100 transition-opacity" />
                      ) : (
                        <X className="size-4" />
                      )}
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        disabled={saving}
                        className="inline-flex items-center gap-2 rounded-xl bg-slate-900 text-white px-4 py-2 text-sm font-medium shadow-sm hover:shadow disabled:opacity-50"
                      >
                        {saving ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />} Confirm booking
                      </button>
                      <button
